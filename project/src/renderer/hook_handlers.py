...
def loop_handler(params: list) -> None:
    ...
    if renderer.renderer_queue[0] == "background":
        renderer.draw(
            image,
            renderer.animations["elements"]["background"]["target"],
            {
                "background": renderer.animations["elements"]["background"]["color"]
            }
        )
        renderer.draw(
            image,
            [renderer.animations["elements"]["fourtytwo"]["target"]],
            {
                "fourtytwo":renderer.animations["elements"]["fourtytwo"]["in_color"]
            }
        )
        ...
        current = next(
            renderer.animations["elements"]["walls"]["target"],
            None
        )
        if current is not None:
            renderer.draw(
                ...
                [[current]],
                {
                    "walls": renderer.animations["elements"]["walls"]["color"]
                }
            )
        else:
            renderer.renderer_queue.pop(0)
        ...
        renderer.draw(
            image,
            [renderer.animations["elements"]["exit"]["target"]],
            {
                "exit": renderer.animations["elements"]["exit"]["in_color"]
            }
        )
        renderer.draw(
            image,
            [renderer.animations["elements"]["entry"]["target"]],
            {
                "entrance": renderer.animations["elements"]["entry"]["in_color"]
            }
        )
        renderer.renderer_queue.pop(0)
    elif renderer.renderer_queue[0] == "path":
        current = next(
            renderer.animations["elements"]["path"]["target"],
            None
        )
        if current is not None:
            renderer.draw(
                ...
                [[current]],
                {
                    "path": renderer.animations["elements"]["path"]["in_color"]
                }
            )
        else:
            renderer.renderer_queue.pop(0)
    ...
    if not renderer.animations["elements"]["path"]["on"]:
        ...
        for c, d in sol_path:
            ...
            renderer.draw(
                ...
                [[(c, d)]],
                {
                    "path": renderer.animations["elements"]["background"]["color"]
                }
            )
...
def key_handler_controller(keycode: int, params: AppResources) -> int:
    if keycode == 65307: # ESC key
        exit_loop(params.context.mlx_ptr)
        return 0
